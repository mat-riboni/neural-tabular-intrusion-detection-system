...
def main(args):
    ...
    unsup = TabNetPretrainer(
        ...
        cat_emb_dim=cat_emb_dims, mask_type='sparsemax',
        optimizer_fn=torch.optim.Adam,
        optimizer_params=dict(lr=2e-2),          
        device_name=device, seed=RANDOM_STATE,
    )

    logger.info('--- PRE-TRAINING (unsupervised) ---')
    unsup.fit(
        X_train=X_train_processed.values,
        eval_set=[X_val_processed.values],
        max_epochs=training_params.get("pretrain_epochs", 80),
        patience=training_params.get("pretrain_patience", 15),
    ...
    clf.fit(
        ...
        loss_fn=loss_fn, from_unsupervised=unsup
    )
